...
public class MainActivity extends Activity
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        if (app.accList.size() == 0) {
            ...
            accCfg.setIdUri("sip:12345678@25.30.9.134:8060");
            accCfg.getRegConfig().setRegistrarUri("sip:123456@25.30.9.134:8060");
            accCfg.getNatConfig().setIceEnabled(true);
            accCfg.getVideoConfig().setAutoTransmitOutgoing(true);
            accCfg.getVideoConfig().setAutoShowIncoming(true);

            accCfg.getRegConfig().setRetryIntervalSec(10L);
            accCfg.getRegConfig().setFirstRetryIntervalSec(3L);
            accCfg.getRegConfig().setRandomRetryIntervalSec(7L);
        ...
        try {
            CodecInfoVector codecInfoVector = Endpoint.instance().codecEnum();
            StringBuilder sb = new StringBuilder();
            for (int i = 0; i < codecInfoVector.size(); i++) {
                sb.append(codecInfoVector.get(i).getCodecId() + "\n");
            }
            tv.setText(sb);
        } catch (Exception e) {
            e.printStackTrace();
...
//        account = app.addAcc(accCfg);
        ...
        cfg.setUri("sip:123456@25.30.9.134:8060");
